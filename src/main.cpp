...
int main(int argc, char **argv) {
    if (argc > 1)   // If an argument is passed
        bport = (int) strtol(argv[1], NULL, 10);    // The number given as an argument becomes the listening port of the main server
    cout << "Server started... Listening on port " << bport << " ..." << endl;  // Prints the listening port
    thread t1(query);
    t1.join();
