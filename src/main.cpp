		...
		if (FD_ISSET(STDIN_FILENO, &rdfs))  // If something happens on standard input
			...
			for (k = 0; k < actual; k++) {
				if (FD_ISSET(clients[k].sock, &rdfs)) { // A client is talking
					...
					} else if (startsWith(buffer, "onlinep")) {
						send_info_to_client(clients[k], "Online Players", qStats.onlinePlayers);
					} else if (startsWith(buffer, "onlineq")) {
						send_info_to_client(clients[k], "Online Queries", qStats.onlineQueries);
					} else if (startsWith(buffer, "playedg")) {
						send_info_to_client(clients[k], "Played Games", qStats.playedGames);
					} else if (startsWith(buffer, "currentg")) {
						send_info_to_client(clients[k], "Current Playing Games", qStats.currentGames);
					} else if (startsWith(buffer, "totalp")) {
						send_info_to_client(clients[k], "Total Players", qStats.totalPlayers);
					} else if (buffer == "close") {
						clear_clients(clients, actual);
						end_connection(sock);
						return;
...
void welcome() {
	cout << "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n"
			"-            Welcome to            *\n"
			"*      The Brickest Dungeon        -\n"
			"-   Made by The Dankest Trinome    *\n"
			"*   Tutored by Manos Katsomallos   -\n"
			"-           Loved by You           *\n"
			"*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-" << endl;
}
...
int main(int argc, char **argv) {
	...
	usleep(10000);  // Waits before running the main server so the query server can start right
